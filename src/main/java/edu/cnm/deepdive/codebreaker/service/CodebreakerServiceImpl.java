...
class CodebreakerServiceImpl implements CodebreakerService {
  ...
  public CompletableFuture<Game> getGame(String gameId) {
    ...
    api.getGame(gameId).enqueue(new Callback<>() {
      @Override
      public void onResponse(Call<Game> call, Response<Game> response) {
        if (response.isSuccessful()) {
          future.complete(response.body());
  ...
  public CompletableFuture<Guess> submitGuess(String gameId, Guess guess) {
    ...
    api.submitGuess(gameId, guess).enqueue(new Callback<>() {
      ...
      @Override
      public void onFailure(Call<Guess> call, Throwable throwable) {
        future.completeExceptionally(throwable);
      }
    });
  ...
  public CompletableFuture<Guess> getGuess(String gameId, String guessId) {
    ...
    api.getGuess(gameId, guessId).enqueue(new Callback<>() {
      @Override
      public void onResponse(Call<Guess> call, Response<Guess> response) {
        if (response.isSuccessful()) {
          future.complete(response.body());
  ...
  private static CodebreakerApi buildApi(Properties properties, Gson gson, OkHttpClient httpClient) {
